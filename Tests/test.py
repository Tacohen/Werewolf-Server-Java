...
r = requests.post(voteElaineUrl, auth=HTTPBasicAuth(username, password),data=data, headers=headers)
r = requests.get(checkAllAliveUrl,auth=HTTPBasicAuth(username, password))
print(r.text)
print("Notice Elaine now has one vote against her, while Joshua has 0. \n")
time.sleep(1)
print("Now Elaine votes for Joshua.. \n")
r = requests.post(voteJoshuaUrl, auth=HTTPBasicAuth(username, password),data=data, headers=headers)
r = requests.get(checkAllAliveUrl,auth=HTTPBasicAuth(username, password))
print(r.text)
