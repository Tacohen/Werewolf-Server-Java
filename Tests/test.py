...
voteElaineUrl = 'http://powerful-depths-2851.herokuapp.com/players/vote/elaine/jake'
voteJakeUrl = 'http://powerful-depths-2851.herokuapp.com/players/vote/jake/elaine'
...
r = requests.post(setDayUrl, auth=HTTPBasicAuth(username, password),data=data, headers=headers)
print("Is it night?")
r = requests.get(isNightUrl,auth=HTTPBasicAuth(username, password))
print(r.text)
print("Now that it is day, let's have voting begin! Jake obviously votes for Elaine the werewolf, and Elaine votes for Jake. \n")
r = requests.post(voteJakeUrl, auth=HTTPBasicAuth(username, password),data=data, headers=headers)
r = requests.post(voteJakeUrl, auth=HTTPBasicAuth(username, password),data=data, headers=headers)
r = requests.get(checkAllAliveUrl,auth=HTTPBasicAuth(username, password))
print(r.text)
